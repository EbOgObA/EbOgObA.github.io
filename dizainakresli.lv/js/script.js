$(document).ready(function () {
    ...
    function meklet() {
        $('.meklet_button').on('click', function(e) {
            e.preventDefault();
            $('.meklet_input').addClass('active');
        });
        $(document).click(function(evt) {
            if (evt.target.id == "meklet" || evt.target.id == "search_i_2") {
                return;
            } else if ($(evt.target).closest('#meklet').length) {
                return;
            } else {
                $("#meklet .meklet_input").removeClass('active');
                $("#meklet .meklet_input").val("");
            }
        });
    }
    meklet();
