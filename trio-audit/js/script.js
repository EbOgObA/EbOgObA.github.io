document.addEventListener('DOMContentLoaded', function () {
    ...
    const scrolling = (e) => {
        if (scrollEnable === 1) {
            ...
            let index = $('.active-section').index();
            if (index == 0) {
    ...
    const slide = (index) => {
        ...
        // console.log(index, +(windowHeight * index * (-1)));
        $('main').css({ 'top': +(windowHeight * index * (-1)) });
        classActiveInd();
        classActiveSection();
    ...
    const classActiveInd = () => {
        $indicator.removeClass('active-ind');
        $indicator.eq($currentSection.index()).addClass('active-ind');
    }
    ...
    $('section').bind('mousewheel', scrolling);
